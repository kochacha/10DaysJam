...
void KochaEngine::Player::SetMiniMap(MiniMap* arg_miniMap)
{
	pMiniMap = arg_miniMap;
}
...
void KochaEngine::Player::UpdateAfterSmashPos()
{
	...
	int bCount = (smashPower * lsam.amountReductionItem) + (overDirveSmashPower * lsam.amountReductionSpine); //ここの倍率で戻る量が変わる
	if (backCount > 0)
	{
		bCount = backCount;
	}
